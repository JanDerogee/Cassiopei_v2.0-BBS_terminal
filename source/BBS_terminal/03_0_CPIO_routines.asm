...
;CPIO_BACKOFF    
;                LDY #100        ;number of times the loop below should run
...
;BCKOFF_01       TYA             ;
;                PHA             ;
;                JSR UPDATE_KEYBUF       ;force keyboard update during backoff waiting period (otherwise the system will no be as responsive as it should)
;                PLA             ;
;                TAY             ;
;                DEY             ;
;                BNE BCKOFF_01   ;
...
CPIO_BACKOFF    
                LDY #10         ;number of times the loop below should run
BCKOFF_01       LDX #255        ;
                TYA             ;
                PHA             ;
                JSR UPDATE_KEYBUF       ;force keyboard update during backoff waiting period (otherwise the system will no be as responsive as it should)
                PLA             ;       ;But... don't poll it too quickly, otherwise the key bouncing will be noticeable
                TAY             ;
BCKOFF_02       PHA             ;3 cycles (1 cycle is 1,0149729 us (PAL))
                PLA             ;4 cycles
                DEX             ;2 cycles
